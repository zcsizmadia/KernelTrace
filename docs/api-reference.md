...
## dotnet-kerneltrace CLI  *(Feature 9)*

A .NET global tool for command-line eBPF tracing.
...
Exits `0` if kernel BTF is available, `1` otherwise.

#### `trace`

```
dotnet-kerneltrace trace <probe-object.bpf.o> \
    --probe-type tracepoint|kprobe|kretprobe|usdt \
    --name       <probe-name>        \
    [--category  <category>]         \  # required for tracepoint
    [--pid       <pid>]              \  # -1 = all (default)
    [--limit     <n>]                \  # stop after n events (default: 50)
    [--btf-path  <file.btf>]         \  # custom BTF archive
    [--debug]                        \  # verbose libbpf output
    [--binary    <path>]             \  # required for usdt
    [--provider  <name>]               # required for usdt
```

Streams raw events as `<ISO-8601-timestamp> <HEX>` lines to stdout.

#### `map-dump`

```
dotnet-kerneltrace map-dump <probe-object.bpf.o> <map-name> \
    --probe-name <name>        \
    [--category  <category>]
```

Attaches a minimal session and prints all map entries as `KEY → VALUE` hex pairs.

#### `kallsyms-resolve`

```
dotnet-kerneltrace kallsyms-resolve <addr> [<addr> ...]
```

Resolves one or more hex kernel addresses to symbol names using `/proc/kallsyms`.
